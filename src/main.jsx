...
function App() {
  ...
  return <div className="app"><aside>
    ...
    <nav>{[['总览',LayoutDashboard],['家庭账本',WalletCards],['成长健康',HeartPulse],['用品库存',PackageCheck]].map(([label,Icon]) => <button key={label} className={active===label?'active':''} onClick={()=>setActive(label)}><Icon size={18}/>{label}{active===label&&<i/>}</button>)}</nav>
    ...
    <div className="content">
      ...
      <section className="hero"><div><div className="hero-label"><Baby size={16}/> 鱼蛋成长档案</div><div className="hero-title">今天也在认真长大<br/><strong>每一天都值得记录</strong></div><p>数据来自鱼蛋看板，页面仅用于查看。</p></div><div className="milestone"><div className="rings"><ShieldCheck size={22}/><small>只读</small></div><div><span>数据保护</span><b>不提供新增、编辑或删除操作</b></div></div></section>
      ...
      <section className="grid lower">
        <div className="panel chart-panel"><div className="panel-head"><div><h3>家庭支出趋势</h3><span>近 6 个月 · 单位：元</span></div><div className="legend"><i className="orange"/>支出 <i className="gray"/>收入</div></div><Chart months={months}/></div>
