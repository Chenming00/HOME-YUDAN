...
家庭只读数据总览看板，以体重记录为当前关注重点，将「鱼蛋小账本」「成长与疫苗记录」「宝宝消耗品库存」三个上游数据源聚合成一屏家庭仪表板。
...
- **四板块看板**
  - **今天（TODAY）**：体重数值与趋势优先展示；可切换「体重成长 / 疫苗儿保 / 生活准备」，当前关注保存在本机，方便随成长阶段调整。
...
- **响应式布局**：手机底部导航、iPad 紧凑侧栏、PC 完整侧栏；按可用宽度布局，支持平板分屏。
- **体重展示**：首页与成长页共用轻量 SVG 图表，支持近 12 次、近 30 次、全部记录；历史记录每次显示 20 条。图表按实际记录日期排列，支持触摸与键盘查看数值。
- **首屏加载**：成长、账本和库存页面按需加载；使用系统字体；15 秒请求超时，刷新失败保留当前记录。打开记录本会跳转到现有上游系统，看板仍只读。
...
运行本地开发服务后，可使用已有的 Playwright 环境运行：

```sh
node scripts/verify-dashboard.mjs
```

可通过 `TEST_URL` 指定待测试的本地地址，`PLAYWRIGHT_MODULE` 指定 Playwright 模块的文件 URL，`BROWSER_CHANNEL` 选择本机已安装的 `chrome` 或 `msedge`。这些仅用于测试，无新增生产依赖。

脚本在测试浏览器中拦截聚合请求并提供合成数据，不修改任何真实记录。覆盖 320–1920px 的 12 组宽度、关注切换及偏好保存、体重图表坐标与键盘交互、库存搜索、加载状态、刷新失败、未连接数据源、单笔记录、长文本和历史分页。截图输出到已忽略的 `.dev-shots/`。浏览器尺寸模拟不能替代 iPhone/iPad Safari 真机验收。
